#Chord diagram/ circular plot
#https://www.r-graph-gallery.com/chord-diagram/
#https://www.r-graph-gallery.com/123-circular-plot-circlize-package-2/
...
# dat <- with(dat, table(name, feature)) #change list into matrix
# 
...
m <- m[,-(1:2)]/1e04
m <- as.matrix(m[,c(1:6)])
dimnames(m) <- list(orig = df1$country, dest = df1$country) #m:matrix between countries
